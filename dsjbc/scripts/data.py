...
# 1. 设定下载股票代码为苹果公司（AAPL），时间跨度从2015年到2024年
ticker = "AAPL"
print(f"正在下载 {ticker} 的历史数据...")

stock = yf.Ticker(ticker)
# history 方法会自动返回包含 Open, High, Low, Close, Volume, Dividends, Stock Splits 的数据
df = stock.history(start="2015-01-01", end="2024-01-01")

# 2. 调整重置索引，让日期（Date）变成普通的一列
df = df.reset_index()

# 3. 只保留我们需要的核心字段：日期、收盘价、最高价、最低价、开盘价、成交量、股息
# 这样能让格式非常干净，便于 Hive 解析
